...
""" A simple script to visualize the SIFT descriptor for an interactively drawn
    sketch """
    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            for k in range(8):
                ...
                v = O[i+j*4, k]/10.0
                end_point = ((i+0.5)*V.shape[0]/4.0+v*cos(theta),
                             (j+0.5)*V.shape[0]/4.0-v*sin(theta))

                # compute the end points of the lines on the arrow head
                arrow_e1 = (end_point[0]+cos(theta+3*pi/4)*v/4.0,
                            end_point[1]-sin(theta+3*pi/4)*v/4.0)
                arrow_e2 = (end_point[0]+cos(theta-3*pi/4)*v/4.0,
                            end_point[1]-sin(theta-3*pi/4)*v/4.0)
                ...
                cv2.line(V,
                         (int(end_point[0]), int(end_point[1])),
                         (int(arrow_e1[0]), int(arrow_e1[1])),
                         2)
                cv2.line(V,
                         (int(end_point[0]), int(end_point[1])),
                         (int(arrow_e2[0]), int(arrow_e2[1])),
                         2)


def mouse_event(event, x, y, flag, dc):
    """ handle mouse events, basically lets you sketch by
        clicking in the left pane """
    global drawing
    global last_x
    global last_y
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.line(im,
                 (int(x/(visualizer_size[1]/patch_size[1])),
                  int(y/(visualizer_size[0]/patch_size[0]))),
                 (int(last_x/(visualizer_size[1]/patch_size[1])),
                  int(last_y/(visualizer_size[0]/patch_size[0]))),
                 0,
                 2)
...
if __name__ == '__main__':
    ...
    extractor = detector
    # older versions of of OpenCV might need the following lines instead o fhte
    # ones above
    ...
    keypoint.octave = 1245440   # multiple things are encoded  using bitmasking
    keypoint.size = 40.0        # the precision of the Gaussian window
    # rotation invariance is from the keypoint not the descriptor
    keypoint.angle = 0.0

...
class ObjectTracker(object):
    ...
    def track(self,im):
        ...
        for idx in range(len(good_matches)):
            ...
            track_im[int(training_keypoints[match[1]].pt[1]),
                     int(training_keypoints[match[1]].pt[0])] = 1.0
        ...
        for max_iter in range(1, 10):
            ...
            (ret, intermediate_roi) = cv2.meanShift(track_im,
                                                    track_roi,
                                                    term_crit)
            cv2.rectangle(track_im_visualize,(intermediate_roi[0],
                                              intermediate_roi[1]),
...
def mouse_event(event,x,y,flag,im):
    if event == cv2.EVENT_FLAG_LBUTTON:
        if tracker.state == tracker.SELECTING_QUERY_IMG:
            ...
            cv2.circle(tracker.query_img_visualize,
                       (x, y),
                       5,
                       (255, 0, 0),
                       5)
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar('Corner Threshold',
                       ...
                       0,
                       100,
                       set_corner_threshold_callback)
    cv2.createTrackbar('Ratio Threshold',
                       'UI',
                       100,
                       100,
                       set_ratio_threshold_callback)
    ...
    while not rospy.is_shutdown():
        ...
        if tracker.state == tracker.SELECTING_QUERY_IMG:
            if tracker.query_roi != None and tracker.query_descriptors is not None:
                ...
                combined_img = np.zeros((frame.shape[0],
                                         frame.shape[1]+(tracker.query_roi[2]-tracker.query_roi[0]),frame.shape[2]),dtype=frame.dtype)
                ...
                for pt in tracker.query_keypoints:
                    cv2.circle(combined_img,
                               (int(pt.pt[0]+frame.shape[1]), int(pt.pt[1])),
                               2,
                               (255,0,0),
                               1)
                cv2.rectangle(combined_img,
                              (tracker.last_detection[0],
                               tracker.last_detection[1]),
                              (tracker.last_detection[2],
                               tracker.last_detection[3]),
                              (0,0,255),
                              2)

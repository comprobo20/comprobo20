...
class MarkerLocator(object):
    def __init__(self, id, position, yaw=0.0):
...
class MarkerProcessor(object):
    def __init__(self, use_dummy_transform=False):
        ...
        m_per_tile = 2*12*2.54/100.0
        self.add_marker_locator(MarkerLocator(0,(6*m_per_tile,0.0)))
        self.add_marker_locator(MarkerLocator(1,(3*m_per_tile,0.0)))
        self.add_marker_locator(MarkerLocator(2,(0.0,0.0)))
        self.add_marker_locator(MarkerLocator(3,(6*m_per_tile,3*m_per_tile)))
        self.add_marker_locator(MarkerLocator(4,(3*m_per_tile,3*m_per_tile)))
        self.add_marker_locator(MarkerLocator(5,(0.0,3*m_per_tile)))
        self.add_marker_locator(MarkerLocator(6,(6*m_per_tile,6*m_per_tile)))
        self.add_marker_locator(MarkerLocator(7,(3*m_per_tile,6*m_per_tile)))
        self.add_marker_locator(MarkerLocator(8,(0.0,6*m_per_tile)))
        ...
        self.marker_sub = rospy.Subscriber("/camera/tag_detections",
    ...
    def process_markers(self, msg):
        for marker in msg.detections:
            ...
            if (marker.id in self.marker_locators and
                ...
                pose_stamped = PoseStamped(header=Header(stamp=marker.pose.header.stamp,frame_id="STAR"),pose=pose)

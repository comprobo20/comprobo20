function [level, xcob, ycob] = cumulativeVolume(allSlices,deltaZ,desiredVolume)
% TODO: comments
...
%   This yields level = 0.2307, xcob = 0.5, ycob = 0.1094
    ...
    for i = 1 : length(allSlices)
        ...
        edges = [edges; points(1:end-1,:) points(2:end,:)];
    end
    ...
    lb = min(allPoints(:,2));
    ub = max(allPoints(:,2));
    if isinf(desiredVolume)
        level = ub;
        [~,xcob,ycob] = areaHelper(level,true);
        return;
    end
    
    if areaHelper(ub)*deltaZ < desiredVolume
        % impossible
        return
    end
    ...
    levelVolume = Inf;    % ensure we always run the loop once by initializing to large value
    % TODO might be able to optimize further with sorting and bookkeeping
    while abs(levelVolume - desiredVolume) > 10^-4
        level = (ub + lb)/2;
        levelVolume = areaHelper(level)*deltaZ;
        if levelVolume > desiredVolume
            ub = level;
        else
            lb = level;
        end
